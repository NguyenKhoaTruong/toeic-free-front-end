...
import { uploadFileQuizListening } from '../services/userService';
import { toast } from 'react-toastify';


class AddCsvQuizListening extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showModal: false,
            smShow: false,
            mode: "addCsvQuizListening",
            isOpen: false,
            selectedFile: null
    ...
    onClickHandler = async (datafile) => {
        try {
            if (datafile) {
                ...
                await uploadFileQuizListening(data);
            ...
            <div>
                ...
                >
                    <Modal.Header closeButton={true}>
                        <h2>{this.state.mode === "addCsvQuizListening" && "Thêm Dữ Liệu Câu Hỏi Phần Nghe  Bằng File Excel"}</h2>
                    </Modal.Header>
                    <Modal.Body>
                        {this.state.mode === "addCsvQuizListening" && (this.renderAddCsvQuizListening())}
                    </Modal.Body>
                    <Modal.Footer>
...
export default connect(mapStateToProps, mapDispatchToProps)(AddCsvQuizListening);
