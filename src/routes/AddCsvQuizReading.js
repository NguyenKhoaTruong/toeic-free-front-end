...
import { uploadFileQuizReading } from '../services/userService';
import { toast } from 'react-toastify';


class AddCsvQuizReading extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showModal: false,
            smShow: false,
            mode: "addCsvQuizReading",
            isOpen: false,
            selectedFile: null
    ...
    onClickHandler = async (datafile) => {
        try {
            if (datafile) {
                ...
                await uploadFileQuizReading(data);
            ...
            <div>
                ...
                >
                    <Modal.Header closeButton={true}>
                        <h2>{this.state.mode === "addCsvQuizReading" && "Thêm Dữ Liệu Câu Hỏi Phần Đọc  Bằng File Excel"}</h2>
                    </Modal.Header>
                    <Modal.Body>
                        {this.state.mode === "addCsvQuizReading" && (this.renderAddCsvQuizReading())}
                    </Modal.Body>
                    <Modal.Footer>
...
export default connect(mapStateToProps, mapDispatchToProps)(AddCsvQuizReading);
