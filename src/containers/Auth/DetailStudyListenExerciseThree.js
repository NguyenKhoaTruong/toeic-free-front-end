...
class DetailStudyListenExerciseThree extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getAllQuizListening = async () => {
        try {
            let response = await getAllQuizListening('PART 3');
            if (response && response.errCode === 0) {
                this.setState({
                        ...
                        <div className="col-2"><div className='border left-tet ml-3' style={{ width: '-webkit-fill-available', cursor: 'pointer' }} data-tut="reactour__listtablecontent">
                            <div className='option-study-for-test' data-tut="reactour__listtablecontent">
                                ...
                                {arrReadExercise && arrReadExercise.length > 0 && arrReadExercise.map((item, index) => {
                                    return (
                                        <>
                                            {item.part === 'PART 1' && <div className='study-test-one border' onClick={() => this.handleShowStudy(item.id)} data-tut="reactour__option_5">
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(DetailStudyListenExerciseThree));
