...
class ListStudyPartTwo extends Component {
    constructor(props) {
        super(props);
        this.state = {
            arrReadExercise: []
    ...
    handleDetailListenExercise = (dataListenExercise) => {
        ...
        this.props.history.push(`/luyen-thi-part-2/${dataListenExercise.id}`)
    }
    render() {
                            ...
                            <div className='content-testonline border' >
                                ...
                                <div className='list-exam-test'>
                                    {arrReadExercise && arrReadExercise.length > 0 && arrReadExercise.map((item, index) => {
                                        return (
                                            <>
                                                {item && item.part === 'PART 2' && <div className='exam-test' key={index} onClick={() => this.handleDetailListenExercise(item)}>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(ListStudyPartTwo));
