...
import './StudyForTestPart1.scss';
import ListStudyPartOne from './ListStudyPartOne';
...
import DetailStudyListenExercise from './DetailStudyListenExercise';
import LoginForm from '../../routes/LoginForm';
class StudyForTestPart1 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isTourOpen: false,
    ...
    render() {
        let { isTourOpen, isShowingMore, showluyenthi, showluyenthi1, showTipStudy, arrResultTest, showModalLoginExam,
            showStudy, arrReadExercise, showList, arrPartOne
        } = this.state;
        let { isLoggedIn, userInfo } = this.props;
        const accentColor = "#5cb7b7";
        console.log(' ádasdasdasdasd  ', arrReadExercise)
        console.log('check arrResultTest', arrResultTest)
        let listenId = '';
        {
                                    ...
                                    <div className='border left-tet ml-3'>
                                        <div className='option-study-for-test' data-tut="reactour__listtablecontent">
                                            ...
                                            {arrReadExercise && arrReadExercise.length > 0 && arrReadExercise.map((item, index) => {
                                                return (
                                                    <>
                                                        {item.part === 'PART 1' && <div className='study-test-one border' onClick={() => this.handleShowStudy(item.id)} data-tut="reactour__option_1">
                                        ...
                                        <div className='content-testonline' style={{ width: '100%' }} >
                                            ...
                                            <div className='list-exam-test'>
                                                {arrReadExercise && arrReadExercise.length > 0 && arrReadExercise.map((item, index) => {
                                                    return (
                                                        <>
                                                            {item && item.part === 'PART 1' && <div className='exam-test' key={index} onClick={() => this.handleDetailListenExercise()}>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(StudyForTestPart1));
