...
class DetailStudyListenExerciseSeven extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getAllQuizReading = async () => {
        try {
            let response = await getAllQuizReading('PART 7');
            if (response && response.errCode === 0) {
                this.setState({
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(DetailStudyListenExerciseSeven));
